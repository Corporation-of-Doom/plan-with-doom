...
export function loadEvents(id) {
  ...
  return fetch({
    ...
    if (res.data) {
      ...
      eventInfo.start_time = moment(parseInt(eventInfo.start_time, 10)).format(
        "MMMM Do YYYY, h:mm a"
      );
      eventInfo.end_time = moment(parseInt(eventInfo.end_time, 10)).format(
        "MMMM Do YYYY, h:mm a"
      );
      eventInfo.announcements.forEach(event => {
        event.date_modified = moment(parseInt(event.date_modified, 10)).format(
          "MMMM Do YYYY, h:mm a"
        );
      });
      eventInfo.seminars.forEach(seminar => {
        seminar.start_time = moment(parseInt(seminar.start_time, 10)).format(
          "MMMM Do YYYY, h:mm a"
        );
        seminar.end_time = moment(parseInt(seminar.end_time, 10)).format(
          "MMMM Do YYYY, h:mm a"
        );
      });
      ...
      if (
        user.follow.filter(
          item => item.__typename === "Event" && item.id === id
        ).length > 0
      ) {
      ...
      eventInfo.creator_id = eventInfo.organizers.filter(
        organizer => organizer.id === eventInfo.creator_id
      )[0].name;
      eventInfo.id = id;
...
export function loadSeminars(id) {
  ...
  return fetch({
    ...
    if (res.data) {
      ...
      seminarInfo.start_time = moment(
        parseInt(seminarInfo.start_time, 10)
      ).format("MMMM Do YYYY, h:mm a");
      seminarInfo.end_time = moment(parseInt(seminarInfo.end_time, 10)).format(
        "MMMM Do YYYY, h:mm a"
      );
      seminarInfo.announcements.forEach(seminar => {
        seminar.date_modified = moment(
          parseInt(seminar.date_modified, 10)
        ).format("MMMM Do YYYY, h:mm a");
      });
      ...
      return fetch({
        ...
        if (
          user.follow.filter(
            item => item.__typename === "Seminar" && item.id === id
          ).length > 0
        ) {
        ...
        if (
          user.attend.filter(
            item =>
              item.__typename === "Event" && item.id === seminarInfo.event_id
          ).length > 0
        ) {
